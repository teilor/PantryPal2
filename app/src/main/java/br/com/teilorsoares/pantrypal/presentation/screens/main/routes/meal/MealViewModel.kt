...
class MealViewModel(
    private val getFoodSortedByMealUseCase: GetFoodSortedByMealUseCase,
...
) : ViewModel<MealUiState, MealUiAction>(MealUiState()) {
    ...
    private fun collectFood(consumed: Boolean = false) {
        ...
        collectFood = viewModelScope.launch {
            ...
            getFoodSortedByMealUseCase(consumed)
                .flowOn(ioDispatcher)
                .collect { data ->
                val groups = data
                    .map { map ->
    ...
    private suspend fun updateFoodVisibility(
        ...
        try {
            setState { state ->
                ...
                val groupIndex = groups.indexOfFirst { it.meal == food.meal }
